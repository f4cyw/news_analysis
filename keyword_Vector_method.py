...
def train_word2vec_model(sentences):
    # Ensure sentences are properly tokenized for Word2Vec
    bigram = Phrases(sentences, min_count=1, threshold=2)
    bigram_sentences = [bigram[sentence] for sentence in sentences]
    tokenized_sentences = [list(sentence) for sentence in bigram_sentences if len(sentence) > 0]
    
    if not tokenized_sentences:
        raise ValueError("No valid sentences found for training. Please provide more meaningful input.")
    
    # Initialize and build vocabulary
    model = Word2Vec(vector_size=100, window=5, min_count=1, workers=4)
    model.build_vocab(tokenized_sentences)  # Build vocabulary before training
...
def visualize_graph(G):
    ...
    nx.draw_networkx(G, pos, with_labels=True, node_size=1500, node_color='skyblue', font_size=10, font_weight='bold', width=edge_widths, edge_color=edge_colors, edge_cmap=plt.cm.plasma, alpha=0.8, font_family=font_prop.get_name())
    plt.title("Keyword Relationship Graph (Word2Vec)", fontsize=16)
    plt.show()
...
"""
    main(articles, additional_keywords=['주가'])